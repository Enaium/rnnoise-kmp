...
val rnnoiseDir = rootProject.projectDir.resolve("rnnoise")
...
kotlin {
    ...
    targets.withType<KotlinNativeTarget> {
        ...
        compilations.getByName("main") {
            cinterops {
                create("rnnoise") {
                    defFile(project.file("src/nativeInterop/cinterop/rnnoise.def"))
                    includeDirs(
                        project.file("src/nativeInterop/cinterop"),
                        rootProject.file("rnnoise/include"),
                    )
                    if (canBuild) {
...
// subprojects. Their JARs ship librnnoise_jni as a classpath resource that
...
// System.loadLibrary directly from Rnnoise.jvm.kt, so they still need a flat
// directory containing librnnoise_jni.{so,dylib}. We reuse the host's
...
fun registerNativeBuildTasks(targetName: String, cmakeFlags: List<String> = emptyList()) {
    ...
    tasks.matching {
        it.name.startsWith("cinteropRnnoise") &&
...
mavenPublishing {
    ...
    pom {
        name.set("rnnoise-kmp")
        description.set(
            "Kotlin Multiplatform bindings for the RNNoise noise suppression library.",
        )
